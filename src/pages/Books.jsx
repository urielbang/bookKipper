...
export default function Books() {
  ...
  const [booksNotBerrowed, setBooksNotBerrowed] = useState([]);
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    setBooksNotBerrowed([...booksNotBerrowed, newBook]);
  };
  const handleClick = () => {
    setShowForm(!showForm);
  };
  const renderBooks = booksNotBerrowed.map((book) => {
    return (
      <BookCard
        key={book.id}
        book={book}
        setBooksNotBerrowed={setBooksNotBerrowed}
        booksNotBerrowed={booksNotBerrowed}
      />
    );
  });
  const handleSerch = (e) => {
    const updtedBooks = books.filter((book) => {
      return book.name.includes(e.target.value);
    });
    setBooksNotBerrowed(updtedBooks);
  };

  useEffect(() => {
    const neBooks = books.sort((a, b) => a.name.localeCompare(b.name));
    const BooksNotBorrowed = neBooks.filter((book) => {
      return book.isBorrowed == false;
    });
    setBooksNotBerrowed(BooksNotBorrowed);
  ...
  return (
    <div>
      ...
      <NavBarCategories
        ...
        setBooksNotBerrowed={setBooksNotBerrowed}
      />
      <div className="containerSerachBooks">
        <input
          className="searchBooks"
          onChange={handleSerch}
          type="text"
          name=""
          placeholder="searchBookByName"
        />
      </div>
      <div className="containerBooks">{renderBooks}</div>
    </div>
  );
}
